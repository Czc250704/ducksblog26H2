/**
 * DucksBlog26H2 - 前端主逻辑
 * 后端 API 地址：Cloudflare Worker
 */

// ==================== 配置 ====================
const API_BASE = 'https://ducksblog-api.xsh3304832000.workers.dev/api';
...
async function fetchData(filename) {
    return await apiFetch(`/data/${filename}`);
}
...
async function getFileUrl(filePath) {
    const data = await apiFetch(`/file/${encodeURIComponent(filePath)}`);
    return data.url;
}
...
async function previewFile(filename, type) {
    ...
    try {
        ...
        container.innerHTML = '<div class="text-center py-10 text-red-500">预览失败</div>';
    }
}
...
function updateUIForAdmin() {
    ...
    if (adminBtn) adminBtn.classList.toggle('hidden', isLoggedIn);
    if (logoutBtn) logoutBtn.classList.toggle('hidden', !isLoggedIn);
    if (roleBadge) roleBadge.innerText = isLoggedIn ? (isMaster ? '最高管理员' : '二级管理员') : '';
    
    if (state.currentCategory && document.getElementById('filesView')?.classList.contains('hidden') === false) {
        const uploadPanel = document.getElementById('uploadPanel');
        if (uploadPanel) uploadPanel.classList.toggle('hidden', !isLoggedIn);
    }
    const newCatBtn = document.getElementById('newCategoryBtn');
    const delCatBtn = document.getElementById('deleteCategoryBtn');
    if (newCatBtn) newCatBtn.classList.toggle('hidden', !isMaster);
    if (delCatBtn) delCatBtn.classList.toggle('hidden', !isMaster || !state.currentCategory);
    const approvalPanel = document.getElementById('approvalPanel');
    if (approvalPanel) approvalPanel.classList.toggle('hidden', !isMaster);
    if (isMaster) {
        renderApprovalPanel();
        renderSystemSettings();
    }
...
async function submitUpload() {
    ...
    const typeMap = {
        md:'md', txt:'txt', ppt:'ppt', pptx:'pptx', doc:'doc', docx:'docx', 
        xls:'xls', xlsx:'xlsx', pdf:'pdf', jpg:'jpg', jpeg:'jpeg', png:'png', gif:'gif'
    };
...
function renderSystemSettings() {
    ...
    if (toggle) toggle.checked = state.settings.globalPasswordEnabled;
    if (pwdGroup) pwdGroup.style.display = state.settings.globalPasswordEnabled ? 'block' : 'none';
    if (pwdInput) pwdInput.value = state.settings.globalPassword || '';
    ...
    if (saveBtn) {
        saveBtn.onclick = async () => {
            if (toggle) state.settings.globalPasswordEnabled = toggle.checked;
            if (toggle && pwdInput) {
    ...
    if (toggle) {
        toggle.addEventListener('change', () => {
            if (pwdGroup) pwdGroup.style.display = toggle.checked ? 'block' : 'none';
        });
    }
}
...
function initModalControls() {
    ...
    if (minimizeBtn) minimizeBtn.onclick = () => modal.classList.add('hidden');
...
function bindEvents() {
    ...
    if (newCategoryBtn) newCategoryBtn.addEventListener('click', createNewCategory);
    if (deleteCategoryBtn) deleteCategoryBtn.addEventListener('click', deleteCurrentCategory);
    if (uploadSubmit) uploadSubmit.addEventListener('click', submitUpload);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (savedUser) {
        try {
            ...
            else if (ADMIN_CONFIG.sub.some(s => s.username === u.username)) {
                state.currentUser = { ...ADMIN_CONFIG.sub.find(s => s.username === u.username), role: 'sub' };
            }
        } catch(e) {}
    }
    await loadAllData();
